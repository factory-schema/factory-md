...
[![Version](https://img.shields.io/badge/spec-v0.4%20(beta)-orange.svg)](SPEC.md)

**Factory.md** is an open standard (v0.4, beta) that makes manufacturing facilities machine-readable. A single Markdown file with YAML frontmatter describes a factory's capabilities, certifications, and callable skills — readable by humans, AI agents, and procurement platforms alike. Structured frontmatter for discovery and filtering, rich Markdown body for everything else.
...
Four fields are required: `schema`, `name`, `location`, and `vertical`. Everything else — capabilities, certifications, contact, last-updated date, inventory/RFQ flags, and `skills` — is recommended. The Markdown body contains rich descriptions of quality processes, equipment, tolerances, compliance, and more.
...
3. **Validate** the frontmatter against the [JSON Schema](schema/v0.4/factory.schema.json)
...
| [schema/v0.4/factory.schema.json](schema/v0.4/factory.schema.json) | JSON Schema v0.4 (Draft 2020-12) — validates YAML frontmatter |
